...
Future<void> statusCommand(String target) async {
  ...
  try {
    ...
    if (state.dimming != null) {
      stdout.writeln('  Brightness: ${state.dimming}%');
    }
    if (state.isRgbMode) {
      stdout.writeln('  Color: RGB(${state.r}, ${state.g}, ${state.b})');
    }
    if (state.temperature != null) {
      stdout.writeln('  Temperature: ${state.temperature}K');
    }
    if (state.scene != null) {
      stdout.writeln('  Scene: ${state.scene!.displayName}');
    }
