...
async def list_lights():
    config = load_config()
    lights = [
        LightResponse(ip=light.ip, name=name, alias=light.alias, mac=light.mac)
...
async def discover_lights(timeout: float = 5.0, broadcast: str = "255.255.255.255"):
    ...
    config = load_config()
    discovered = [LightResponse(ip=l.ip, mac=l.mac, name=l.name) for l in new_lights]
    return DiscoveryResponse(discovered=discovered, new_count=len(new_lights), total_count=len(config.lights))
...
async def toggle(light_id: str):
    try:
        ...
        await controller.close()
        return SuccessResponse(success=True, message=f"Light {light_id} toggled to {new_state.value}")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConnectionError as e:
        raise HTTPException(status_code=503, detail=str(e))
...
async def set_brightness(light_id: str, request: BrightnessRequest):
    try:
        ...
        await controller.close()
        return SuccessResponse(success=True, message=f"Brightness set to {request.value}%")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConnectionError as e:
        raise HTTPException(status_code=503, detail=str(e))
...
async def set_color(light_id: str, request: ColorRequest):
    try:
        ...
        await controller.close()
        return SuccessResponse(success=True, message=f"Color set to RGB({request.r}, {request.g}, {request.b})")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConnectionError as e:
        raise HTTPException(status_code=503, detail=str(e))
...
async def set_temperature(light_id: str, request: TemperatureRequest):
    try:
        ...
        await controller.close()
        return SuccessResponse(success=True, message=f"Temperature set to {request.kelvin}K")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConnectionError as e:
        raise HTTPException(status_code=503, detail=str(e))
...
async def apply_scene(light_id: str, request: SceneRequest):
    try:
        ...
        return SuccessResponse(success=True, message=f"Scene '{request.scene}' applied")
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConnectionError as e:
        raise HTTPException(status_code=503, detail=str(e))
